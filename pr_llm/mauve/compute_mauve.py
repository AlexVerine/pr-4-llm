...
def compute_mauve(
    ...
    * ``out.divergence_curve`` contains the points in the divergence curve. It is of shape (m, 2), where m is ``divergence_curve_discretization_size``
    ...
    alpha, beta = pr_curves(p.reshape(-1, 1), q.reshape(-1, 1))
    precision = np.max(_prd_to_f_beta(alpha, beta, 1 / 8))
    recall = np.max(_prd_to_f_beta(alpha, beta, 8))

    to_return = SimpleNamespace(
        hist={"p": p, "q": q},
        divergence_curve={"x": x, "y": y},
        mauve=mauve_score,
        frontier_integral=fi_score,
        num_buckets=num_buckets,
        prcurve={"alpha": alpha, "beta": beta},
        precision=precision,
        recall=recall,
    )
    return to_return
...
def cluster_feats(
    ...
    """Cluster features using k-means and return the normalized histogram of cluster assignments.
    :param p: numpy.ndarray of shape (n, d), where n is the number of generations.
    :param q: numpy.ndarray of shape (n, d), where n is the number of generations.
    :param num_clusters: number of clusters to use.
    :param norm: normalization to use. Options: ``'none'``, ``'l2'``, ``'l1'``.
    :param whiten: whether to whiten the data before clustering.
    :param pca_max_data: the number data points to use for PCA. If `-1`, use all the data. Default -1.
    :param explained_variance: amount of variance of the data to keep in dimensionality reduction by PCA. Default 0.9.
    :param num_redo: number of times to redo k-means clustering (the best objective is kept). Default 5.
        Try reducing this to 1 in order to reduce running time.
    :param max_iter: maximum number of k-means iterations. Default 500.
        Try reducing this to 100 in order to reduce running time.
    :param seed: random seed to initialize k-means cluster assignments.
    :param verbose: If True, print running time updates.
    """
...
def pr_curves(p, q, N_bins=1000):
    alpha, beta = [], []
    for theta in range(1, N_bins):
        alpha.append(
            np.min(
                np.concatenate((np.tan(theta * np.pi / 2 / N_bins) * p, q), axis=1),
                axis=1,
            ).sum()
        )
        beta.append(
            np.min(
                np.concatenate((p, q / np.tan(theta * np.pi / 2 / N_bins)), axis=1),
                axis=1,
            ).sum()
        )
