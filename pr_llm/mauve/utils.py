...
def load_and_tokenize_json_data(
    tokenizer, data_path, max_len=1024, max_num_data=float("inf")
):
    ...
    """
    assert (
        max_len <= 1024 and max_num_data >= 2000
...
@torch.no_grad()
def featurize_tokens_from_model(
    model,
    tokenized_texts,
    batch_size,
    name="",
    ...
    for chunk, chunk_sent_length in tqdm(
        list(zip(chunks, chunk_sent_lengths)), desc=f"Featurizing {name}", leave=False
    ):
        ...
        h = []
        if layer == 0.5:
            layer = len(outs.hidden_states) // 2
        for hidden_state, sent_length in zip(
            outs.hidden_states[layer], chunk_sent_length
        ):
            if "gpt" in model_name:
                h.append(hidden_state[sent_length - 1])
            elif "bert" in model_name:
                h.append(hidden_state[0])
